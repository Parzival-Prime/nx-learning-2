  ...
  return (
    <div className='flex justify-center items-center min-h-[80vh] px-4 my-10'>
      ...
      >
        ...
        <div className='bg-neutral-100 p-4 rounded-md text-sm text-neutral-700 space-y-3'>
            {cartItems.map((item, idx)=>(
                <div key={idx} className="flex justify-between text-sm pb-1">
                    <span>
                        {item.quantity} * {item.title}
                    </span> ${(item.quantity * item.sale_price).toFixed(2)}
                </div>
            ))}

            <div className="flex justify-between font-semibold pt-2 border-t border-t-neutral-300">
